...
Code block gets wrapped into a function and sent to real GCC which builts a shared library that is immediately loaded.

If the code has a function definition, the compiled function gets stored for later use.
But to call function compiled in a previous call, a special api has to be used.
Same with variables, to persist them between different calls, a special api has to be used, normal variables are not persisted.
